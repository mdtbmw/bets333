...
The core of the application are the `IntuitionBettingOracle` and `UserProfileRegistry` smart contracts. These contracts must be deployed to the target blockchain before the front-end application can be deployed.
...
- Uses the `INTUITION_DEPLOYER_PRIVATE_KEY` to sign and send the deployment transactions.
- Passes the `TREASURY_ADDRESS` and `PLATFORM_FEE_BPS` as constructor arguments to the betting contract.
- Upon success, it prints the newly deployed contract addresses to the console.

**CRITICAL NEXT STEP:** You **must** copy the outputted contract addresses and paste them into your `.env` file as the values for `NEXT_PUBLIC_INTUITION_BETTING_ADDRESS` and `NEXT_PUBLIC_USER_PROFILE_REGISTRY_ADDRESS`.
...
- **Smart Contract Addresses:** The `NEXT_PUBLIC_INTUITION_BETTING_ADDRESS` and `NEXT_PUBLIC_USER_PROFILE_REGISTRY_ADDRESS` in your `.env` file are the link between the off-chain front-end and the on-chain logic. The one-click deploy script forces a verification of this step.
- **Contract Verification:** To make your contract source code readable on the block explorer, you can run the following Hardhat commands after deployment.
