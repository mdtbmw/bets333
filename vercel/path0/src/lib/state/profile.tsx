...
export const useProfile = () => {
    ...
    const fetchProfile = useAtom(fetchProfileAtom)[1]; // The second element is the write function

    return { profile, isLoading, fetchProfile };
}

// Factory function to create atoms, ensuring they are recreated per user session
const createProfileAtoms = (address: string | null | undefined): ProfileContextType => {
...
export const ProfileProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchProfile = useAtom(profileAtoms.fetchProfileAtom)[1];
